...
def wt(vimg, filters, levels=1):
    ...
    if levels > 1:
        res = wt(res[:, :, :16, :16], filters, levels - 1)

    res = res.view(-1, 2, h // 2, w // 2).transpose(1, 2).contiguous().view(-1, 1, h // 2, w // 2)
    return res.reshape(bs, -1, h // 2, w // 2)
...
def wt_16(vimg, filters, levels=1):
  bs = vimg.shape[0]
  h = vimg.size(2)
  w = vimg.size(3)
  vimg = vimg.reshape(-1, 1, h, w)
...
def wt_8(vimg, filters, levels=1):
  bs = vimg.shape[0]
  h = vimg.size(2)
  w = vimg.size(3)
  vimg = vimg.reshape(-1, 1, h, w)
